```@meta
EditURL = "https://github.com/trixi-framework/Trixi.jl/blob/main/NEWS.md"
```
...
- New time integrator `PairedExplicitRK3`, implementing the third-order paired explicit Runge-Kutta
  ...
  and [NLsolve.jl](https://github.com/JuliaNLSolvers/NLsolve.jl) ([#2008](https://github.com/trixi-framework/Trixi.jl/issues/2008))
- `LobattoLegendreBasis` and related datastructures made fully floating-type general,
  enabling calculations with higher than double (`Float64`) precision ([#2128](https://github.com/trixi-framework/Trixi.jl/issues/2128))

## Changes when updating to v0.9 from v0.8.x

#### Added

- Boundary conditions are now supported on nonconservative terms ([#2062](https://github.com/trixi-framework/Trixi.jl/issues/2062)).

#### Changed

- We removed the first argument `semi` corresponding to a `Semidiscretization` from the 
  `AnalysisSurfaceIntegral` constructor, as it is no longer needed (see [#1959](https://github.com/trixi-framework/Trixi.jl/issues/1959)).
  The `AnalysisSurfaceIntegral` now only takes the arguments `boundary_symbols` and `variable`.
  ([#2069](https://github.com/trixi-framework/Trixi.jl/issues/2069))
- In functions `rhs!`, `rhs_parabolic!`  we removed the unused argument `initial_condition`. ([#2037](https://github.com/trixi-framework/Trixi.jl/issues/2037))
...
- Nonconservative terms depend only on `normal_direction_average` instead of both 
  ...
  `flux_nonconservative_powell` ([#2062](https://github.com/trixi-framework/Trixi.jl/issues/2062)).
...
- The AMR routines for `P4estMesh` and `T8codeMesh` were changed to work on the product
  ...
  to make AMR fully conservative ([#2028](https://github.com/trixi-framework/Trixi.jl/issues/2028)). This may change AMR results slightly.
...
- The specification of boundary names on which `AnalysisSurfaceIntegral`s are computed (such as drag and lift coefficients) has changed from `Symbol` and `Vector{Symbol}` to `NTuple{Symbol}`.
  Thus, for one boundary the syntax changes from `:boundary` to `(:boundary,)` and for `Vector`s `[:boundary1, :boundary2]` to `(:boundary1, :boundary2)` ([#1959](https://github.com/trixi-framework/Trixi.jl/issues/1959)).
- The names of output files like the one created from the `SaveSolutionCallback` have changed from `%06d` to `%09d` to allow longer-running simulations ([#1996](https://github.com/trixi-framework/Trixi.jl/issues/1996)).
...
#### Added
- Implementation of `TimeSeriesCallback` for curvilinear meshes on `UnstructuredMesh2D` and extension to 1D and 3D on `TreeMesh` ([#1855](https://github.com/trixi-framework/Trixi.jl/issues/1855), [#1873](https://github.com/trixi-framework/Trixi.jl/issues/1873)).
- Implementation of 1D Linearized Euler Equations ([#1867](https://github.com/trixi-framework/Trixi.jl/issues/1867)).
- New analysis callback for 2D `P4estMesh` to compute integrated quantities along a boundary surface, e.g., pressure lift and drag coefficients ([#1812](https://github.com/trixi-framework/Trixi.jl/issues/1812)).
- Optional tuple parameter for `GlmSpeedCallback` called `semi_indices` to specify for which semidiscretization of a `SemidiscretizationCoupled` we need to update the GLM speed ([#1835](https://github.com/trixi-framework/Trixi.jl/issues/1835)).
- Subcell local one-sided limiting support for nonlinear variables in 2D for `TreeMesh` ([#1792](https://github.com/trixi-framework/Trixi.jl/issues/1792)).
- New time integrator `PairedExplicitRK2`, implementing the second-order paired explicit Runge-Kutta
  method with [Convex.jl](https://github.com/jump-dev/Convex.jl) and [ECOS.jl](https://github.com/jump-dev/ECOS.jl) ([#1908](https://github.com/trixi-framework/Trixi.jl/issues/1908))
- Add subcell limiting support for `StructuredMesh` ([#1946](https://github.com/trixi-framework/Trixi.jl/issues/1946)).
...
- Experimental support for artificial neural network-based indicators for shock capturing and
  adaptive mesh refinement ([#632](https://github.com/trixi-framework/Trixi.jl/issues/632))
- Experimental support for direct-hybrid aeroacoustics simulations ([#712](https://github.com/trixi-framework/Trixi.jl/issues/712))
...
- Composable `FluxPlusDissipation` and `FluxLaxFriedrichs()`, `FluxHLL()` with adaptable
  wave speed estimates were added in [#493](https://github.com/trixi-framework/Trixi.jl/issues/493)
...
- `flux_lax_friedrichs(u_ll, u_rr, orientation, equations::LatticeBoltzmannEquations2D)` and
  ...
  in [#493](https://github.com/trixi-framework/Trixi.jl/issues/493). This is considered a bugfix
  (released in Trixi.jl v0.3.22).
- The required Julia version is updated to v1.6.

#### Deprecated

- `calcflux` → `flux` ([#463](https://github.com/trixi-framework/Trixi.jl/issues/463))
...
- The old interface for nonconservative terms is deprecated. In particular, passing
  ...
  nonconservative equations, see [#657](https://github.com/trixi-framework/Trixi.jl/issues/657).

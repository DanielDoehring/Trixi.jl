...
Refinement = 7 # Use something reasonably fine for the initial set-up
...
analysis_callback = AnalysisCallback(semi, interval=analysis_interval,
                                     extra_analysis_errors=(:conservation_error, :l1_error, :cell_center_error))
...
amr_controller = ControllerThreeLevel(semi, TrixiExtension.IndicatorVortex(semi),
                                      base_level=2,
                                      med_level=5, med_threshold=-3.0,
                                      max_level=8, max_threshold=-2.0)

CFL_Stab = 1.0
amr_callback = AMRCallback(semi, amr_controller,
                           interval=Int(10/CFL_Stab), 
...
sol = solve(ode, 
            SSPRK22(thread = OrdinaryDiffEq.True()),

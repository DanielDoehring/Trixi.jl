...
struct SSP104_2Nstar <: SimpleAlgorithm2Nstar
    ...
    function SSP104_2Nstar()
        ...
        c = SVector(0.0, 0.580177480734878, 0.849381888003236, 0.949226744603282, 0.937361313412918, 0.252585824034633,
                     0.438296526814728, 0.644001345007665, 0.875228468077886, 1.00381542908022)
...
struct SSP53_2Nstar <: SimpleAlgorithm2Nstar
    ...
    function SSP53_2Nstar()
        ...
        A = SMatrix{5,5}(0.0000000000000000, 0.0000000000000000, 0.0000000000000000, 0.0000000000000000, 0.0000000000000000,
                        0.3772689153349139, 0.0000000000000000, 0.0000000000000000, 0.0000000000000000, 0.0000000000000000,
                        0.3772689153210351, 0.3772689153210356, 0.0000000000000000, 0.0000000000000000, 0.0000000000000000,
                        0.2273646445334142, 0.2273646445166876, 0.2273646445250521, 0.0000000000000000, 0.0000000000000000,
                        0.2118911342119499, 0.1277410428919109, 0.1277410428966103, 0.2119622635173820, 0.0000000000000000)
        A = A'
        c = Vector{Float64}(undef, 5)
        for i in 1:5
            c[i] = sum(A[i,j] for j in 1:5)
        end
        b = SVector(0.2256709383174730, 0.1170972518383502, 0.1170972518426580, 0.1943008917840403, 0.3458336662174786)
...
function SimpleIntegrator2NstarOptions(callback, tspan; maxiters = typemax(Int), kwargs...)
...
function init(ode::ODEProblem, alg::SimpleAlgorithm2Nstar;
    ...
    t = first(ode.tspan)
    iter = 0
    integrator = SimpleIntegrator2Nstar(u, du, u_tmp, du_tmp, t, dt, zero(dt), iter, ode.p,
                                    (prob = ode,), ode.f, alg,
                                    SimpleIntegrator2NstarOptions(callback, ode.tspan;
                                                        kwargs...), false)
...
function step!(integrator::SimpleIntegrator2Nstar)
    ...
    # one time step
    integrator.u_tmp .= integrator.u  # q2 = q1
    t_stage = integrator.t
    integrator.f(integrator.du, integrator.u, prob.p, t_stage)  # f(q1)
    integrator.du_tmp .= integrator.du
    integrator.u = integrator.u + alg.gamma[1]*integrator.du*integrator.dt  # q1 = q1 + y_21*f(q1)
    for stage in 2:length(alg.c)
        t_stage = integrator.t + integrator.dt * alg.c[stage]  # t = t + h*c_i
        integrator.f(integrator.du, integrator.u, prob.p, t_stage)  # f(q1)
        
        lambda_stage = alg.lambda[2*(stage-1)] # first column of lambda
        gamma_stage_1_dt = alg.gamma[2*(stage-1)+1]*integrator.dt  # subdiagonal of gamma
        gamma_stage_2_dt = alg.gamma[2*(stage-1)]*integrator.dt  # first column of gamma
                     
        @trixi_timeit timer() "Runge-Kutta step" begin
            @threaded for i in eachindex(integrator.u)
                integrator.u[i] = lambda_stage*integrator.u_tmp[i] + (1-lambda_stage)*integrator.u[i] + 
                                gamma_stage_1_dt*integrator.du[i] + gamma_stage_2_dt*integrator.du_tmp[i]
            end
        end
    end
...
get_tmp_cache(integrator::SimpleIntegrator2Nstar) = (integrator.u_tmp, integrator.du_tmp)

...
function relaxation_solver!(integrator,
    ...
    add_direction!(u_tmp_wrap, u_wrap, dir_wrap, gamma_1, dg, cache)
    # `r_1` corresponds to `entropy_residual` in the other methods
    @trixi_timeit timer() "ΔH" r_1=entropy_difference(gamma_1, integrator.S_old, dS,
                                                      u_tmp_wrap, mesh,
                                                      equations, dg, cache)

    add_direction!(u_tmp_wrap, u_wrap, dir_wrap, gamma_min, dg, cache)
    @trixi_timeit timer() "ΔH" r_0=entropy_difference(gamma_0, integrator.S_old, dS,
                                                      u_tmp_wrap, mesh,
                                                      equations, dg, cache)
    ...
    if r_1 > 0 && r_0 < 0
        ...
        while abs(gamma_1 - gamma_0) > gamma_tol && iterations < max_iterations
            add_direction!(u_tmp_wrap, u_wrap, dir_wrap, gamma_1, dg, cache)
            r_0 = r_1
            @trixi_timeit timer() "ΔH" r_1=entropy_difference(gamma_1, integrator.S_old,
                                                              dS, u_tmp_wrap, mesh,
                                                              equations, dg, cache)
            ...
            if integrator.gamma < gamma_min
                integrator.gamma = 1
                add_direction!(u_tmp_wrap, u_wrap, dir_wrap, integrator.gamma, dg,
                               cache)
                @trixi_timeit timer() "ΔH" r_1=entropy_difference(integrator.gamma,
                                                                  integrator.S_old,
                                                                  dS, u_tmp_wrap,
                                                                  mesh, equations,
                                                                  dg, cache)
    ...
    # Update old entropy
    integrator.S_old += integrator.gamma * dS + r_1

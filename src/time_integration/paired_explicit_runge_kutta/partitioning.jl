...
function dtmax_per_element(u, mesh::P4estMesh{2}, equations, dg, cache)
    n_nodes = nnodes(dg)
    n_elements = nelements(dg, cache)
    ...
    for element in eachelement(dg, cache)
        max_a1 = max_a2 = zero(eltype(u))
        #max_a_total = zero(eltype(u))
        for j in eachnode(dg), i in eachnode(dg)
            u_node = get_node_vars(u, equations, dg, i, j, element)

            # Speeds
            a1, a2 = max_abs_speeds(u_node, equations)
        ...
        dtmax_per_element_[element] = 1 / (max_a1 + max_a2)

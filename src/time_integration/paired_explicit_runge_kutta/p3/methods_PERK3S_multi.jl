...
struct PairedExplicitRK3SplitMulti <:
       AbstractPairedExplicitRKSplitMulti{3}
...
function PairedExplicitRK3SplitMulti(stages::Vector{Int64},
    ...
    a_matrices, c,
    max_active_levels,
    max_add_levels = compute_PairedExplicitRK3Multi_butcher_tableau(stages, num_stages,
    ...
    max_add_levels_para_ = compute_PairedExplicitRK3Multi_butcher_tableau(stages_para,
                                                                          # Need to supply `num_stages` here to have matching Butcher tableaus
                                                                          num_stages,
                                                                          base_path_mon_coeffs_para,
                                                                          cS2)

    return PairedExplicitRK3SplitMulti(length(stages), num_stages,
                                       length(stages_para), num_stages_para,
                                       dt_ratios, dt_ratios_para,
                                       a_matrices, a_matrices_para, c,
...
function init(ode::ODEProblem, alg::PairedExplicitRK3SplitMulti;
    ...
    du_para = zero(u0)
    integrator = PairedExplicitRK3SplitMultiIntegrator(u0, du, u_tmp,
                                                       t0, tdir,
                                                       dt, zero(dt),
                                                       iter, semi,
                                                       (prob = ode,),
...
function Base.resize!(integrator::PairedExplicitRK3SplitMultiIntegrator,
                      new_size)

...
@inline function flux_differencing_kernel!(_du::PtrArray, u_cons::PtrArray,
    ...
    for i in eachnode(dg), ii in (i + 1):nnodes(dg)
        @turbo for j in eachnode(dg)
            rho_ll = u_cons[1, i, j, element]
            rho_v1_ll = u_cons[2, i, j, element]
            rho_v2_ll = u_cons[3, i, j, element]
            rho_e_total_ll = u_cons[4, i, j, element]
            ...
            rho_rr = u_cons[1, ii, j, element]
            rho_v1_rr = u_cons[2, ii, j, element]
            rho_v2_rr = u_cons[3, ii, j, element]
            rho_e_total_rr = u_cons[4, ii, j, element]
            ...
            normal_direction_1 = 0.5 * (contravariant_vectors[1, 1, i, j, element] +
                                  contravariant_vectors[1, 1, ii, j, element])
            normal_direction_2 = 0.5 * (contravariant_vectors[2, 1, i, j, element] +
                                  contravariant_vectors[2, 1, ii, j, element])
            ...
            # Add scaled fluxes to RHS
            factor_i = alpha * derivative_split[i, ii]
            du[i, j, 1] += factor_i * f1
            du[i, j, 2] += factor_i * f2
            du[i, j, 3] += factor_i * f3
            du[i, j, 4] += factor_i * f4

            factor_ii = alpha * derivative_split[ii, i]
            du[ii, j, 1] += factor_ii * f1
            du[ii, j, 2] += factor_ii * f2
            du[ii, j, 3] += factor_ii * f3
            du[ii, j, 4] += factor_ii * f4
        end
    end
    ...
    for j in eachnode(dg), jj in (j + 1):nnodes(dg)
        @turbo for i in eachnode(dg)
            ...
            rho_rr = u_cons[1, i, jj, element]
            rho_v1_rr = u_cons[2, i, jj, element]
            rho_v2_rr = u_cons[3, i, jj, element]
            rho_e_total_rr = u_cons[4, i, jj, element]
            ...
            normal_direction_1 = 0.5 * (contravariant_vectors[1, 2, i, j, element] +
                                  contravariant_vectors[1, 2, i, jj, element])
            normal_direction_2 = 0.5 * (contravariant_vectors[2, 2, i, j, element] +
                                  contravariant_vectors[2, 2, i, jj, element])
